...
export const isProjectAlignedToScrollTarget = (container: HTMLElement, projectUrl: string) => {
...
export const getActiveProjectUrl = ({
  ...
  const activationLine =
    container.getBoundingClientRect().top + getScrollPaddingTop(container) + ACTIVE_LINE_OFFSET
  let currentHeading = headings[0]
  let closestDistance = Number.POSITIVE_INFINITY

  for (const heading of headings) {
    const distance = Math.abs(heading.getBoundingClientRect().top - activationLine)
